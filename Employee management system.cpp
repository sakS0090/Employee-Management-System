...
int password_user()
{
    ...
    if(selection == 1)
    {
        ...
        getch();
        return password_user();  // Re-prompt for login after registration
    }
    else if(selection == 2)
    {
        ...
        while(file >> name_reg >> pass_reg)
        {
            if(pass == pass_reg && name == name_reg)
            {
                ...
                cout<<"User logged in on   "<<asctime(localTime);
                file.open("Ep_data.txt", ios::in);
                file<<asctime(localTime);
                file.close();
                system("PAUSE");
                accessGranted = true;
                break;
        ...
        if (!accessGranted)
        {
            ...
            cout << "\n\n Access Aborted.. \n\n";
            system("PAUSE");
            return password_user();  // Re-prompt for login on failure
        }
